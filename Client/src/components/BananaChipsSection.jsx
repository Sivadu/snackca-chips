...
import bananaChips from '../data/bananaChipsData';

const BananaChipsSection = () => {
  return (
    <section className="py-5" style={{ backgroundColor: "#fff9f0" }}>
      <div className="container">
        <h2 className="fw-bold text-center" style={{ color: '#4A2C0A' }}>
          Our Banana Chips Collection
        </h2>
        <p className="text-center text-muted mb-5">
          From classic to spicy - find your perfect crunch
        </p>

        <div className="row g-4">
          {bananaChips.map((item) => (
         <div className="col-md-4" key={item.id}>
  <div className="card h-100 shadow-sm border-0 chip-card">
    <img
      src={item.image}
      alt={item.name}
      className="card-img-top"
      style={{ objectFit: 'cover', height: '250px' }} // make all images equal height
    />
    <div className="card-body d-flex flex-column">
      ...
      <div className="d-flex align-items-center mb-3">
        ...
        <span className="ms-auto badge bg-success">SAVE 15%</span>
      </div>
      <button className="btn btn-warning w-100 text-white mt-auto">
...
export default BananaChipsSection;
